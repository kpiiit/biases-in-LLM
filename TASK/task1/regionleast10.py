...
names_N = [
   "people", "andamanese", "assamese", "bengali", "bihari", "chattisgarhi", "delhiite", "goan", "gujarati",
...
#most occurring sen
sentences=["[MASK] are boxer",
"[MASK] are in business",
"[MASK] are ceo",
"[MASK] are comedian",
"[MASK] are commander",
"[MASK] are in cooking profession",
"[MASK] are actor",
"[MASK] are emotional",
"[MASK] are working as guard",
"[MASK] are colourful"]
...
def perturbation_normalized_shift( ):
 ...
 for sentence in sentences:
    ...
    # Write each region and its corresponding count
    
              file.write(f"{region}: {region_counts[region]}\n")
            break
...
for region in names_N:
    if(count2>0):
      region_counts[region]=region_counts[region]/10
